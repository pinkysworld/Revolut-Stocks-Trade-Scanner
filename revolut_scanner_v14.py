...
Crypto fixes (you reported v11 produced zero crypto recommendations)
  ...
  • Lowered min-predicted-move floors: weekly 4.0% (was 4.5%),
    daytrade 3.5% (was 4.0%), intraday 3.0% (was 3.5%).  At €100 the
    net profit after 2.98% round-trip fees is still positive:
       weekly   4.0% → gross €4.00, net €1.02
       daytrade 3.5% → gross €3.50, net €0.52
       intraday 3.0% → gross €3.00, net €0.02
...
# --- Crypto sizing (€100 target; doubles absolute P/L vs v11's €50) ---
...
CRYPTO_WEEKLY_MIN_PREDICTED_PCT = 4.0   # was 4.5 — see math in docstring
...
CRYPTO_DAYTRADE_MIN_PREDICTED_PCT = 3.5   # was 4.0
...
CRYPTO_INTRADAY_MIN_PREDICTED_PCT  = 3.0   # was 3.5
...
CRYPTO_OVEREXTENSION_PCT = 30.0   # drop crypto rec if ret5 > this %
...
def market_status(asset_class, ticker, currency, now=None):
    """Return (status, next_open_local, note) for an instrument right now.

    status ∈ {"OPEN", "PRE_MARKET", "AFTER_HOURS", "CLOSED_WEEKEND",
              "CLOSED_HOLIDAY", "CLOSED_OVERNIGHT", "OPEN_24_7"}
    next_open_local: datetime of next open in the instrument's local tz,
                     or None if currently open / 24/7.
    ...
    if asset_class == "crypto":
        return ("OPEN_24_7", None, "24/7 — place anytime")

    # Determine session by ticker convention
    def in_session(open_h, open_m, close_h, close_m, tz_offset_hours):
        ...
        if weekday >= 5:
            ...
            next_open = (local + timedelta(days=days_to_mon)).replace(
                hour=open_h, minute=open_m, second=0, microsecond=0)
            return "CLOSED_WEEKEND", next_open, f"weekend — opens {next_open:%a %H:%M} local"
        ...
        if local < open_dt:
            return "PRE_MARKET", open_dt, f"pre-open — opens today {open_dt:%H:%M} local"
        if local > close_dt:
            # Find next open
            next_day = local + timedelta(days=1)
            while next_day.weekday() >= 5:
                next_day += timedelta(days=1)
            next_open = next_day.replace(hour=open_h, minute=open_m, second=0, microsecond=0)
            return "AFTER_HOURS", next_open, f"closed — opens {next_open:%a %H:%M} local"
        return "OPEN", None, f"open until {close_dt:%H:%M} local"

    # DE stocks (.DE) → XETRA 09:00–17:30 CET (UTC+1, simplified — ignore DST)
    if ticker.endswith(".DE") or ticker.endswith(".AS") or ticker.endswith(".F"):
        return in_session(9, 0, 17, 30, +1)
    # UK FTSE → LSE 08:00–16:30 GMT
    if ticker == "^FTSE":
        return in_session(8, 0, 16, 30, +0)
    # Japan Nikkei → TSE 09:00–15:00 JST (UTC+9)
    if ticker == "^N225":
        return in_session(9, 0, 15, 0, +9)
    # DAX index → 09:00–17:30 CET
    if ticker == "^GDAXI":
        return in_session(9, 0, 17, 30, +1)
    # US stocks / S&P / Nasdaq → NYSE 09:30–16:00 EST (UTC-5)
    if ticker in ("^GSPC", "^NDX") or (not any(ticker.endswith(s) for s in (".DE", ".AS", ".F", "=F"))
                                       and not ticker.startswith("^")):
        return in_session(9, 30, 16, 0, -5)
    # Commodity futures (=F) → Sun 18:00 ET to Fri 17:00 ET with daily 1-hour break
    if ticker.endswith("=F"):
        # Simplified: treat as open Mon–Fri 18:00–17:00 next day ET (~23h/day)
        local = now + timedelta(hours=-5)
        weekday = local.weekday()
        if weekday == 5:   # Saturday
            next_open = local.replace(hour=18, minute=0, second=0, microsecond=0) + timedelta(days=1)
            return "CLOSED_WEEKEND", next_open, "futures weekend — opens Sun 18:00 ET"
        # Most of the time the futures session is open
        return ("OPEN", None, "futures session (24h - 1h daily break)")
    # Fallback — assume US session
    return in_session(9, 30, 16, 0, -5)

def market_note_colored(asset_class, ticker, currency):
    status, next_open, note = market_status(asset_class, ticker, currency)
    if status == "OPEN" or status == "OPEN_24_7":
        return f"{C.GREEN}Market: OPEN — place now{C.RESET}  ({note})"
    if status in ("PRE_MARKET", "AFTER_HOURS"):
        return f"{C.YELLOW}Market: CLOSED — wait{C.RESET}  ({note})"
    if status.startswith("CLOSED"):
        return f"{C.RED}Market: CLOSED{C.RESET}  ({note})"
    return note
...
def load_btc_regime():
    ...
    if not USE_BTC_REGIME_FOR_CRYPTO:
        return None, None
    try:
        df = yf.download(BTC_REGIME_TICKER, period=LOOKBACK, progress=False, auto_adjust=True)
    except Exception:
        return None, None
    if df is None or df.empty or len(df) < 220:
        return None, None
...
def load_and_enrich(ticker, regime_series=None, btc_regime_series=None,
                    btc_close=None, asset_class=None):
    try:
...
def build_crypto_weekly_recommendations(scan_rows, enriched, scores_map,
    ...
    if "crypto" not in (robust_classes | (weak_classes if ALLOW_WEAK_CLASSES else set())):
        if CRYPTO_FILTER_DIAGNOSTIC:
            print(f"{C.YELLOW}[CRYPTO WEEKLY DIAGNOSTIC]{C.RESET} crypto asset class did NOT pass OOS verdict "
                  f"— all {diag['start']} candidates blocked at the gate.")
        return []
    ...
    for r in scan_rows:
        ...
        # Pump filter: don't chase parabolic moves
        key_check = (r["ticker"], r["asset_class"])
        df_check = enriched.get(key_check, (None,))[0]
        if df_check is not None and len(df_check) > 5:
            ret5 = df_check["ret5"].iloc[-1] * 100 if pd.notna(df_check["ret5"].iloc[-1]) else 0
            if ret5 > CRYPTO_OVEREXTENSION_PCT:
                continue  # overextended — likely mean-reverts
...
def build_crypto_intraday_recommendations(crypto_candidates, regime_series):
    ...
    for r in crypto_candidates:
        ...
        for hold_h in CRYPTO_INTRADAY_HOLDS:
            ...
            # Multi-timeframe alignment bonus: when daily + daytrade + intraday all bullish
            mtf_aligned = (USE_MTF_ALIGNMENT
                           and r["score"] >= MTF_ALIGNMENT_THRESHOLD
                           and r["daytrade_score"] >= MTF_ALIGNMENT_THRESHOLD
                           and current["intraday_score"] >= MTF_ALIGNMENT_THRESHOLD)
            if mtf_aligned:
...
def run_scan(iteration=0):
    ...
    for ticker, name, cls, cur in assets:
        df = load_and_enrich(ticker, regime_series, btc_regime_series, btc_close, asset_class=cls)
        if df is None:
